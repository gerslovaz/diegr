...
#' @param type A character indicating the type of color palette to create. Available options: `"redblue"` (default value) for red-blue palette and `"topo"` for topographical palette, see Details for more information.
#' @param symmetric A logical value indicating whether the color range should be strictly symmetrized around zero. Defaults to `TRUE`.
#'
#' @details
...
#'
#' By default (`symmetric = TRUE`), to guarantee true perceptual uniformity and equal visual emphasis, the provided `col_range` is additionally automatically symmetrized around zero based on its maximum absolute value. For example, an input range of `c(-2, 5)` is internally expanded to `c(-5, 5)`.
#'
#' For the default `redblue` palette, this symmetrization ensures that equivalent positive and negative amplitudes are mapped to identical color intensities. Furthermore, the `redblue` palette strictly anchors the zero-point to a neutral background color, preventing false perceptual boundaries. (Note that while the legacy `topo` palette is also symmetrized, it retains its characteristic blue-green transition at zero).
#'
#' Setting `symmetric = FALSE` disables this zero-centering expansion, which may be useful for strictly unipolar data or heavily skewed distributions, though it sacrifices equal visual emphasis for opposite polarities.
#'
...
ColorsBlue <- function(n,
                       alpha = 1) {
  if (n > 0) {
    pal <- grDevices::colorRampPalette(c("#2E5A87", "#F5F5F5"))
    cols <- pal(n)
...
ColorsRed <- function(n,
                      alpha = 1) {
  if (n > 0) {
    pal <- grDevices::colorRampPalette(c("#F5F5F5", "#A90C38"))
    cols <- pal(n)
