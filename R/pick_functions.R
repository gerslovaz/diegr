...
#'
#' @param coords A data frame, matrix or named tibble with numeric columns of "x" and "y" sensor coordinates. If not defined, the default template coordinates are used.
...
pick_region <- function(coords = NULL,
  ...
  if (!is.null(coords) && is.null(ROI)) {
    stop("When providing custom `coords`, you must also provide a matching `ROI` vector.")
  }

  allowed_hemis <- c("left", "right", "midline")
  allowed_regions <- c("frontal", "central", "parietal", "occipital", "temporal", "face")
  supported_templates <- c("HCGSN256", "biosemi128", "biosemi256", "system1005")
  ...
  if (is.null(template)) {
    ...
    if (!template %in% supported_templates) {
      stop("Unknown template '", template, "'. Supported templates are: ",
           paste(supported_templates, collapse = ", "), ".")
    }
    active_template <- template
  }

  template_data <- switch(active_template,
  ...
  if (!all(c("x", "y") %in% names(coords))) {
    stop("The `coords` object must contain columns 'x' and 'y'.")
  }

  if (!is.numeric(coords$x) || !is.numeric(coords$y)) {
    stop("The 'x' and 'y' columns in `coords` must be numeric.")
  }
