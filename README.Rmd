...
-   animations of time course of raw signals or averages in 2D and 3D (`animate_topo`, `animate_topo_mean`, `animate_scalp`)
...
-   `epochdata`: epoched HD-EEG data (anonymized short slice from big HD-EEG study presented in Madetko-Alster, 2025) for 2 subjects and 204 selected sensors in 50 time points (measured using the EGI HCGSN256 system),
...
This is a basic example which shows how to plot interactive epoch boxplots from a chosen electrode at different time points for one subject:
...
Note: The README format does not allow the inclusion of `plotly` interactive elements, therefore, only a static preview of the result is shown.
...
Compute the average signal for subject 2 from channels E65 and E34 (excluding the oulier epochs 14 and 15) and then display it along with CI bounds (using `plot_time_mean()` conditioned by sensor)
