...
1.  Unfortunately, for MATLAB `.mat` files version 7.3 and higher
    (HDF5-based), there is currently no straightforward way to read
    sensor labels directly in R. Although the labels are stored in
    `data$hdr$label`, the `rhdf5` package cannot properly read MATLAB
    cell arrays containing strings, and the `R.matlab` package does not
    support MAT v7.3 files. Therefore, the sensor labels must be
    obtained using an alternative approach.

2.  The example above follows the *subject–sensor–epoch–time–signal*
