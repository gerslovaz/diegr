...
Function for plotting interactive or static boxplots of EEG amplitude in
individual epochs within the chosen time interval. The function assumes
data from a single subject and a single sensor. Both the static `ggplot`
output and the interactive `plotly` output can be easily customized and
edited, while the interactive `plotly` output additionally allows easy
identification of the epochs corresponding to outliers.
...
A `plotly` or `ggplot2` object with boxplots of EEG amplitude for
individual epochs.
