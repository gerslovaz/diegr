...
Function for plotting interactive or static boxplots of response time in
individual epochs for selected subjects. Both the static `ggplot` output
and the interactive `plotly` output can be easily customized and edited,
while the interactive `plotly` output additionally allows easy
identification of the epoch numbers corresponding to outliers.

If the `condition` column is present in the input data, the boxplots are
...
- interactivity:
  ...
  Logical. Determines whether to render an interactive `plotly` graph
  (default) or a static `ggplot` graph.
