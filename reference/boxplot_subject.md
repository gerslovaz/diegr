...
Function for plotting interactive or static boxplots of EEG amplitude
across subjects for a single epoch and channel, within a specified time
interval. The function assumes data from a single epoch and a single
sensor. Both the static `ggplot` output and the interactive `plotly`
output can be easily customized and edited, while the interactive
`plotly` output additionally allows easy identification of the subjects
...
A `plotly` or `ggplot2` object with boxplots of EEG amplitude across
subjects.
