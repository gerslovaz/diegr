...
M2 <- point_mesh(dim = 2, n = 3000, template = "HCGSN256", type = "circle")
# plotting a mesh - function allows different options of the result plot
plot_point_mesh(M2$D2, col_sensors = "purple", names = TRUE, cex = 0.1)
```
...
topo_plot(data = data_mean, amplitude = "average", col_range = c(-30, 15), contour = TRUE)
...
### Animations
The package includes the `animate_topo()` function for 2D topographic animation in time and `animate_scalp()` function for 3D scalp animation in time. Both of these functions allow to export the output, a gif animation using `gifski` package for 2D case and a .mp4 video format using `av` package for 3D case. 
...
animate_scalp(s1e05, frames_dir = "your_frames_dir_path", output_path = "your_created_video_path.mp4")
```
